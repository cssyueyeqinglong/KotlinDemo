package com.cy.kotlin.demo.util
...
object Base64Utils {
    ...
    init {
        for (i in 0..BASELENGTH - 1) {
            base64Alphabet[i] = -1
        }
        run {
            var i: Int = 'Z'.toInt()
            while (i.compareTo('A'.toInt()) >= 0) {
                base64Alphabet[i] = (i.minus('A'.toInt())).toByte()
                i--
            }
        }
        run {
            var i: Int = 'z'.toInt()
            while (i.compareTo('a'.toInt()) >= 0) {
                base64Alphabet[i] = (i.minus('a'.toInt()) + 26).toByte()
                i--
            }
        }
        ...
        run {
            var i: Int = '9'.toInt()
            while (i.compareTo('0'.toInt()) >= 0) {
                base64Alphabet[i] = (i.minus('0'.toInt()) + 52).toByte()
                i--
            }
        }
        ...
        lookUpBase64Alphabet[62] = '+'.toChar()
        lookUpBase64Alphabet[63] = '/'.toChar()
    ...
    fun encode(binaryData: ByteArray?): String? {
        ...
        for (i in 0..numberTriplets - 1) {
            ...
            val val3 = if (b3.toInt() and SIGN == 0)
    ...
    fun decode(srcData: String): String {
        val decode = decodeSrc(srcData) ?: byteArrayOf()
        var res = String(decode)
        return res
    }
     ...
     */
    fun decodeSrc(encoded: String?): ByteArray? {
